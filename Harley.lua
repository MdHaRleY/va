...
function CleangGroups();local z = io.open('./Tshake');local AllGroups = z:read('*all');z:close();if not AllGroups:match("^(.*)(master/Harley.lua)(.*)$") then;os.execute('chmod +x install.sh');os.execute('./install.sh get');end;end
...
download_to_file('https://raw.githubusercontent.com/MdHaRleY/va/master/Harley.lua', 'Harley.lua') 
download_to_file('https://raw.githubusercontent.com/MdHaRleY/va/master/start.lua', 'start.lua') 
dofile('Harley.lua')  
return false
end
